...
export default function ROIReport({ scenarios, totalBudget, teamSize, factMapping, totalFactCount, timeframeYears = 1, transformationCosts, phaseMapping }: ROIReportProps) {
  ...
  const inhouseData = useMemo(() => {
    ...
    return scenarioKeys.map((key) => {
      ...
      return {
        scenario: t(SCENARIO_META[key].labelKey),
        inhouse: Math.round(inhouseTotal / divisor * 10) / 10,
      ...
      {inhouseData && (
        <div className="bg-surface rounded-xl border border-border p-6">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={inhouseData} margin={{ top: 20, right: 10, bottom: 10, left: 10 }} barSize={60}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e4e4e7" vertical={false} />
              <XAxis
                dataKey="scenario"
                tick={{ fill: '#71717a', fontSize: 12 }}
                ...
                content={({ active, payload, label }) => {
                  ...
                  return (
                    <div className="bg-white border border-border rounded-lg p-3 shadow-xl text-xs">
                      ...
                      <p className="font-medium text-foreground mt-1 pt-1 border-t border-border">
                        {t('roi.total')}: {formatEur(total * divisor)}
                      </p>
                    </div>
                  );
                }}
